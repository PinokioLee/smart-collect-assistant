...
SPEAKER_NOTES = {
    ...
    2: (
        ...
        "② Tree of Thoughts — 처음엔 메일에서 뽑은 항목을 전부 필수컬럼으로 강제(Strict)했는데, 양식이 "
        "부서마다 조금씩 달라서(드리프트) 오탐이 났습니다. 실측으로 Strict 후보는 0.786점에 오탐 "
        "1건, Balanced 후보는 1.0점에 오탐 0건이었습니다. 그래서 후보 3개를 만들어 실제 파일 컬럼과 "
        "비교 평가하는 ToT 방식으로 바꿨습니다. Yao 2023 참고.\n"
        "③ 검증 자체도 처음엔 Azure OpenAI로 엑셀 행을 직접 판단(A)해봤습니다. 정확도는 F1 100%로 "
        "똑같았지만 처리속도가 규칙기반 대비 46행 기준 약 301배, 130행 기준 약 174배 느렸고 호출마다 API "
        "비용이 들고 응답을 JSON으로 못 받으면 파싱이 실패하는 위험도 있었습니다. 그래서 최종 검증· "
        "병합은 pandas·openpyxl 규칙기반(B)으로 갔습니다. 재현성 표준편차는 10회 반복해도 0입니다."
    ),
    3: (
        "[핵심 기술 과제 · 4분]\n"
        ...
        "핵심 판단이었습니다. LLM은 자연어 이해가 필요한 메일 분석에만 쓰고, 결정론이 필요한 검증은 "
        "규칙기반 코드에 맡겼습니다. 감사합니다."
    ),
}
...
def main() -> None:
    ...
    replace_runs(nm["Text 21"], {
        "Fine-tuning, 키워드 검색 등 대안 대비 RAG를 선택한 근거를 서술하세요.":
            "요구항목을 전부 필수컬럼으로 강제(Strict)했더니 양식 드리프트에서 오탐이 났습니다.",
        "검색 정확도, 응답 속도 등 측정 가능한 개선 지표를 포함하세요.":
            "ToT 후보 3개 비교(B)로 Strict 0.786→Balanced 1.0, 오탐 1건→0건 (Yao 2023·Madaan 2023).",
    })
    ...
    replace_runs(nm["Text 13"], {
        "예: Planner Agent가 먼저 전체 계획을 시뮬레이션(MCP 핵심)하고, LangGraph Self-Correction 루프로 계획을 동적 수정하는 Deep Reasoning 구조를 설계했습니다.":
            "LLM은 메일 이해에만, 검증·병합은 규칙기반(B)으로 분리. ① 검증: LLM판단(A) 실측 후 규칙기반 전환 ② 규칙: 고정(A) 대신 ToT 후보비교(B) ③ 교정: 안전 항목만 제한",
        ...
        "단순 방법 대비 이 설계가 필요했던 기술적 근거를 1~2줄로 서술하세요.":
            "정확도가 같으면 비용·속도·재현성이 유리한 쪽을 택함 — LLM은 호출마다 과금·지연 편차·네트워크 의존이 있지만, 규칙기반은 무료·고속·표준편차 0. 검증처럼 상시 반복되는 결정론적 판단일수록 이 차이가 누적됩니다. "
            "결국 ‘LLM에게 자연어 이해를, 코드에게 정확한 계산을’ 맡기는 역할 분리가 이번 과제 전체를 관통하는 설계 원칙이 됐습니다.",
    })
