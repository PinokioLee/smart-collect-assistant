...
SPEAKER_NOTES = {
    ...
    2: (
        ...
        "③ 검증·병합 자체는 LLM이 아닌 pandas·openpyxl 규칙기반입니다. 같은 입력에 같은 결과가 "
        "나와야 신뢰할 수 있어 결정론으로 갔고, 재현성 표준편차가 0입니다."
    ),
    3: (
        "[핵심 기술 과제 · 4분]\n"
        "가장 어려웠던 난제입니다. 단순 규칙검증·단일 LLM 자동수정이 안 되는 이유가 셋입니다. "
        "규칙검증은 오류를 검출만 해 재제출이 반복되고, LLM 자동수정은 비결정성·환각으로 원본 "
        "훼손 위험, 양식 드리프트 시 과잉 필수규칙이 거짓 오류를 양산합니다.\n"
        "해결은 생성과 검증을 분리하고 교정을 결정론화한 것입니다. ToT로 규칙 후보 3개를 만들어 "
        "coverage 게이트로 오탐을 피하고, Self-Correction은 자동교정 후 재검증해 오류가 줄 때만 "
        "채택합니다. 원본은 건드리지 않고 잔여 오류만 재제출을 요청합니다. 추론은 고수준 로그로 "
        "남습니다 — PLAN → ToT BRANCH×3 → EVALUATE → SELECT → CRITIQUE → REVISE → "
        "RE-VALIDATE, 개선될 때만 채택. 논문은 Yao 2023·Madaan 2023·Wang 2023.\n"
        "결과는 수치로 증명합니다. 정답 10건에 검출 F1 100%, 오탐·미탐 0. 자가교정율 100%로 "
        "재작업 60% 감소. 10회 반복해도 재현성 표준편차 0입니다. 감사합니다."
    ),
}
...
def main() -> None:
    ...
    replace_runs(nm["Text 14"], {
        "예: RAG 기반 지식 검색 시스템 + 멀티 에이전트 워크플로우":
            "취합 요청 메일을 분석해 검증기준을 만들고, 제출 엑셀을 규칙기반 검증→자가교정→병합하는 멀티에이전트.",
        "Agent 1 [역할]  →  Agent 2 [역할]  →  Agent 3 [역할]":
            "Requirement → Supervisor(ToT) → Validation → Self-Correction → Report",
    ...
    replace_runs(nm["Text 24"], {
        '"프로젝트의 핵심 가치를 한 문장으로 정의하세요.':
            '"LangGraph 멀티에이전트에 Tree of Thoughts·Self-Correction을 더해,',
        '예: RAG와 멀티 에이전트 기술로 업무 정보 검색 시간을 80% 단축했습니다."':
            '엑셀 취합 검증을 F1 100%·재현성 표준편차 0으로 자동화하고 재작업을 60% 줄였습니다."',
    })
    ...
    replace_runs(nm["Text 21"], {
        "Fine-tuning, 키워드 검색 등 대안 대비 RAG를 선택한 근거를 서술하세요.":
            "단일 규칙 생성은 양식 드리프트에 취약하고, 단순 검증은 오류를 '검출만' 합니다.",
        "검색 정확도, 응답 속도 등 측정 가능한 개선 지표를 포함하세요.":
            "후보 3개 게이트 선택으로 오탐 회피, 자가교정 100%로 재작업 60%↓ (Yao 2023 · Madaan 2023).",
    })
    ...
    replace_runs(nm["Text 27"], {
        "보안, 성능, 비용 등 실제 선택 기준을 명시하세요.":
            "검증·병합은 LLM 비결정성 대신 결정론이 적합 — 재현성과 신뢰를 확보합니다.",
        "Prompt Engineering, Tool Calling 설계 등 핵심 내용을 작성하세요.":
            "4종 규칙(필수/날짜/중복/코드값) 검증, 원본 보존 → 재현성 표준편차 0.",
    })
    ...
    replace_runs(nm["Text 9"], {
        "단순 구현으로 해결되지 않았던 핵심 기술 과제를 제시하세요.":
            "단순 규칙검증·단일 LLM 자동수정으로는 왜 안 되는가?",
        "예: 단순 ReAct Agent는 10단계 이상의 Long-horizon task에서 계획 일관성(Coherence)을 잃고 실패했습니다.":
            "① 규칙검증은 오류를 '검출'만 → 작성자 재제출 반복(병목)  ② LLM 자동수정은 비결정성·환각으로 원본 훼손 위험  ③ 양식 드리프트 시 과잉 필수규칙이 거짓오류(오탐) 양산",
    })
    replace_runs(nm["Text 13"], {
        "예: Planner Agent가 먼저 전체 계획을 시뮬레이션(MCP 핵심)하고, LangGraph Self-Correction 루프로 계획을 동적 수정하는 Deep Reasoning 구조를 설계했습니다.":
            "생성과 검증을 분리하고 교정을 결정론화. ① ToT: 규칙 후보 3개→coverage 게이트 선택(오탐 회피) ② Self-Correction(Self-Refine): 자동교정→재검증→오류 줄 때만 채택 ③ 원본 불변, 잔여 오류만 재제출",
        "Tree of Thoughts / Self-Correction / Deep Reasoning 등 고급 기법을 사용했다면 논문 레퍼런스와 함께 적용 방법을 명시하세요.":
            "고수준 로그: PLAN → ToT BRANCH×3 → EVALUATE → SELECT → VALIDATE → CRITIQUE → REVISE → RE-VALIDATE(개선 시에만 채택)",
        "단순 방법 대비 이 설계가 필요했던 기술적 근거를 1~2줄로 서술하세요.":
            "검증·교정을 결정론화해야 재현성(표준편차 0)과 신뢰를 확보. 논문: ToT(Yao 2023)·Self-Refine(Madaan 2023)·Plan-and-Solve(Wang 2023)",
    })
    # 결과 및 성과 (그룹 내 stat 3개)
    nm = name_map(slides[3])
    replace_runs(nm["Text 18"], {"XX%": "100%"})
    replace_runs(nm["Text 20"], {
        "성과 지표 1": "오류 검출 F1",
        "개선 내용 예: API 호출 성공률 향상": "오탐 0 · 미탐 0 (정답 10건)",
    })
    replace_runs(nm["Text 22"], {"XX%": "60%↓"})
    replace_runs(nm["Text 24"], {
        "성과 지표 2": "재작업 감소",
        "개선 내용 예: 태스크 자동화율 달성": "자가교정율 100% (6/6건)",
    })
    replace_runs(nm["Text 26"], {"XX배": "0"})
    replace_runs(nm["Text 28"], {
        "성과 지표 3": "재현성 표준편차",
        "개선 내용 예: 처리 속도 또는 정확도 향상": "10회 반복 결정론 검증",
    })
