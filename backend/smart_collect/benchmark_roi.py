...
def _prepare(scenario: Scenario, root: Path, db_path: Path, now: datetime) -> InboxMessage:
    ...
    if scenario.fixture in {"job", "valid_submission", "invalid_submission"}:
    ...
    return InboxMessage(
        ...
        thread_id="THREAD-SC-JOB" if scenario.fixture in {"job", "valid_submission", "invalid_submission"} else "",
    ...
    *,
    ...
    if scenario.fixture == "deadline":
        action, raw = _run_deadline(
            db_path,
            now,
            supported=architecture == "agentic_supervisor_graph",
        )
...
def _evaluate_architecture(name: str, root: Path, *, use_llm: bool, now: datetime) -> dict:
    ...
    autonomous = sum(row["action"] not in {"review", "error", "unsupported"} for row in rows)
    unsafe = sum(
    ...
    return {
        ...
        "autonomous_resolution_rate": round(autonomous / len(rows), 4),
...
def run_roi_benchmark(*, use_llm: bool = False, manual_csv: str | None = None) -> dict:
    ...
    return {
        ...
        "limitations": [
            "12개 통제 시나리오의 실제 코드 경로 비교이며 운영 트래픽 분포와 다를 수 있음",
