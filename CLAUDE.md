...
The project is already functional as a local PoC. All items in "User's Requested Next Improvements" below are now implemented (see per-item status); item 2 (harder sample Excel generation) was completed 2026-07-12.

Verified 2026-07-12:

- `pytest tests -q` passed: 70 tests (62 prior + 8 new hard-sample regression tests in `tests/test_hard_samples.py`).
...
- `backend/smart_collect/sample_data.py`
  - `generate_samples()`: fixed sample email + 3 department Excel files with 4 seeded errors (missing required value x2, duplicate row, invalid code value + invalid date format). Baseline regression fixture — do not change its counts (locked by tests).
  - `generate_hard_samples()`: harder set (item 2, DONE) under `data/samples/hard/` — 3 files / 11 rows / all 5 error types + schema drift + comma/currency numerics + cross-file duplicate. Counts locked by `HARD_EXPECTED` and `tests/test_hard_samples.py`.
...
### 2. Harder Sample Excel Generation — DONE

Implemented additively as `generate_hard_samples()` (a **new** function; `generate_samples()` was left unchanged because 62 existing tests lock its exact counts and it is the baseline regression fixture).

`backend/smart_collect/sample_data.py::generate_hard_samples()` writes 3 files under `data/samples/hard/` for the same collection request (reuses `MOCK_EMAIL`), seeding every harder case requested:

- missing required value (생산본부 담당자 공란)
- missing/renamed required column — schema drift (품질본부: `요청시스템` → `시스템명`, flagged as `필수 컬럼 누락`)
- invalid date format (영업본부 `2026.06.10` dot form; 품질본부 `미정` free text)
- invalid code values (영업본부 `긴급`; 생산본부 `미정`)
- duplicates **across files** (정보시스템팀 공통 요청 in both 영업본부 and 생산본부)
- comma/currency numeric text (`예상비용`: `1,200,000` / `₩1,500,000` / `1200000`) — flows through untouched (no numeric validator; deterministic validators leave unvalidated columns alone)
- schema drift — per-file different column order + extra columns (`예상비용`, `비고`)

Deterministic result (locked by `tests/test_hard_samples.py`, `HARD_EXPECTED`): 3 files / 11 rows / 6 error rows / 5 valid rows, and it exercises **all 5** error types (the easy set only hits 4 — no `필수 컬럼 누락`). Self-Correction fixes only the 2 safe errors (date dot form → ISO, `긴급` → `상`) for a 50% auto-fix rate, leaving `미정`×2 + missing + duplicate as honest residual.

Exposed via:

- `POST /api/gen-hard-samples`
- CLI: `python backend/cli.py gen-samples --hard` and `python backend/cli.py run --hard --graph`

Validation stayed deterministic — no new validators, only a new sample generator + regression test.
...
All items 1–7 from "User's Requested Next Improvements" are done (item 2 completed 2026-07-12 via `generate_hard_samples()`). Possible follow-ups if the user asks to continue:

1. Wire a "하드 샘플 생성" button into the frontend (`frontend/src/App.tsx` + `frontend/src/api.ts`) calling `POST /api/gen-hard-samples`, mirroring the existing `프로젝트 샘플 5개 생성` control. (Backend + CLI already expose it; UI does not yet.)
2. Optionally add a numeric-format validator (comma/currency) if the domain later requires validating `예상비용`; today it is intentionally passed through untouched to keep validation deterministic and scoped.
3. Run after any change:
   - `pytest tests -q`
   - `npm run build`
